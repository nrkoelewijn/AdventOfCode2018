...
void setup(){
  input = loadStrings("../day3_1/input3_1.txt");
...
void draw(){
  ...
  //Cross reference the input with all the places where there's overlap.
  for(int i = 0; i < input.length; i++){
    String[] m = match(input[i], "#(\\d+)\\s@\\s(\\d+),(\\d+):\\s(\\d+)x(\\d+)");
    //Fill fabric with rectangle.
    for(int y = (Integer.parseInt(m[3])); y < (Integer.parseInt(m[3]))+(Integer.parseInt(m[5])); y++){
  ...
  //Check the input for any leftover IDs.
  for(int i = 0; i < input.length; i++){
    if(!(input[i].equals("nope"))){
      println(input[i]);
    }
  }
